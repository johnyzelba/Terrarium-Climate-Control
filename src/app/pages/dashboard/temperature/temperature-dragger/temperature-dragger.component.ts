...
export class TemperatureDraggerComponent implements AfterViewInit, OnChanges, OnInit {
  ...
  @Input() fillGrey: string|string[] = '#2ec6ff';
  ...
  @Input() min = 16; // min output value
  @Input() max = 40; // max output value
  @Input() step = 0.1;
  @Input() temperatureOff : boolean;
  ...
  ngOnInit() {
    this.off=this.temperatureOff;
  }
  
  ngAfterViewInit(): void {
    this.fillTemp = this.fillColors;
    // IE fix
    setTimeout(() => {
      this.invalidate();
      this.init = true;
